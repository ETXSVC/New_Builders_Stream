...
def upgrade() -> None:
    ...
    for table in ("vendors", "bom_lines", "bom_line_receipts"):
        ...
        # The `company_id` index every tenant table needs, for the same reason
        # migration 0018 backfilled one onto every table that predated it: the
        # RLS policy just created above puts `company_id IN (SELECT id FROM
        # get_all_descendant_ids(...))` on EVERY query against this table, so
        # without a leading-column index each one sequential-scans. Postgres
        # does not create an index for a foreign key automatically, so the
        # `companies.id` FK above does not supply this.
        #
        # These are in THIS migration rather than a follow-up because
        # `test_company_id_index_coverage.py` sweeps pg_class for every table
        # carrying a `company_id` column — a new tenant table without one fails
        # that gate, by design, so the index is part of creating the table
        # rather than an optimization to add later.
