...
from tests.conftest import TEST_DATABASE_URL, grant_client_access, register_and_login, set_subscription_tier
...
async def _invite_and_login_as(client, admin, role, email):
    ...
    # `email` is carried alongside the headers because migration 0019 binds
    # the signature to the signing account: `POST /estimates/{id}/approve`
    # rejects a `signer_email` that isn't the caller's own, so the approve
    # step below cannot use a hardcoded address.
...
async def _create_and_approve_estimate(
    client, admin, client_role, project_id, markup_profile_id, line_items, *, grant_access=True
):
    """Takes the whole `admin`/`client_role` login dicts rather than just
    their headers, matching tests/test_estimate_approved_handler.py's helper
    of the same name: migration 0019 landed after this file was written and
    added two requirements to a client-role approval — a project-membership
    grant (a client sees nothing on a project they aren't on, and
    `_get_estimate_or_404` 404s rather than 403s so ids can't be enumerated)
    and a `signer_email` matching the signing account.

    `grant_access=False` for a SECOND approval by the same client on the same
    project: membership is per (project, client), not per estimate, and
    `POST /projects/{id}/clients` 409s on a duplicate rather than being
    idempotent. Passed explicitly instead of having this helper swallow the
    409, which would also hide a genuine grant failure.
    """
...
async def test_approving_an_estimate_against_a_bare_lead_creates_no_bom_lines(client):
    ...
    # membership that can reach it (migration 0019) — the project grant the
    # other tests use has nothing to attach to here.
    await grant_client_access(client, admin, lead_id=lead["id"], email=client_role["email"])
