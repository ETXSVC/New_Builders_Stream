...
**Production**: `docker-compose.prod.yml` is the hardened stack (Caddy TLS termination, internal-only DB/Redis, auto-migrations, restart policies, nightly backups) — the full guide, including the server `.env` requirements and the first-deploy smoke-test checklist, is [docs/11-production-deployment.md](docs/11-production-deployment.md).
