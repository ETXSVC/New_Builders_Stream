...
**Status:** Feature-complete against the consolidated requirements (Phases 0–4 of the roadmap), full stack — FastAPI backend plus a Next.js product frontend. The remaining gap between "feature-complete" and "production" is external-service credentials: Stripe and QuickBooks/FreshBooks run against config-selected fake clients until real keys are supplied, and email needs a mail server (the SMTP client is real — `SMTP_HOST` is the switch). See [Build Status](#build-status).
...
| [Builders-Stream-Feature-Status.pdf](docs/Builders-Stream-Feature-Status.pdf) | Every feature against its status, derived from the code rather than from this table. Source: [feature-status.html](docs/feature-status.html); regenerate with `backend/scripts/render_feature_status.py` |
...
| Phase | Status | Notes |
|---|---|---|
| 0 — Foundation | ✅ Done | Multi-tenant schema, RLS, auth (JWT + Argon2id, refresh-token rotation, MFA/TOTP, password reset by email), audit log, CI |
...
| 3 — Accounting/Billing | ✅ Done | Invoices/payments, bills, expenses, auto-drafted deposit + final invoices (event-driven), Stripe subscription lifecycle. `RealStripeClient` is SDK-backed and selected when `STRIPE_API_KEY` is set; the fake remains the default, so tests, CI and a local `docker compose up` need no Stripe account |
| 4 — External Integrations | ✅ Done | QuickBooks/FreshBooks OAuth connect + idempotent sync. Real HTTP clients selected **per provider** on that provider's client id being configured — one can be live while the other is still the fake, since they are separate OAuth applications. Provider-side entity ids are remembered (`integration_entity_mappings`) so a re-sync updates rather than duplicates, and payments sync per payment rather than per settled invoice |
| Profitability reporting | ✅ Done | `/reports`: revenue, cost and margin per project with AR/AP aging and estimated tax liability — the report existed server-side before it had a screen |
| Multi-company membership | ✅ Done | One person may hold memberships in several companies and switch between them in-app. The switch is recorded on the refresh-token chain (`active_company_id`), so it survives the ~14-minute rotation instead of silently reverting mid-task |
| Tenant financial settings | ✅ Done | A tenant's own deposit percentage and tax rate on `/account`, replacing two module constants. Resolved per value: the company's own setting, else its root company's, else the code default — so a head office sets policy once and a branch in another state can still differ |
...
| Team directory | ✅ Done | `/team`: each company's own record of its people — names, address, phone numbers, trade and photo — kept per membership rather than on the global `users` row, so the same person working for two builders has two records neither can read the other's. Company-managed profession list; photo bytes served by a role-checked route rather than off the static volume, which is why the UI fetches them with the session's bearer token instead of pointing an `<img>` at a URL. Product UI at `/team` (list + one person's record), reads `admin`/`project_manager`, writes `admin`. A member edits their own contact details on `/account` via `/team/me` — a separate route with its own request model, so `notes` (the company's private record *about* someone) and `profession` (how the company files them) cannot be self-written, and `notes` is not even returned there |
| Offline estimate capture | ✅ Done | `/estimates/capture`: an estimator on site picks a cached lead or project, a markup profile and catalog items, saves a draft on the device, and it sends itself when signal returns. Cold-starts with no network at all — a service worker replays the whole cached response, headers included, so the document's nonce and its CSP still agree. Nothing is cached until "Make available offline" is pressed, and the caches are keyed by `(user_id, active_company_id)` because RLS stops at the network boundary. A catalog rate that moved while they were out of signal **parks** the draft with old → new per line rather than retrying or silently re-pricing it |
| Field-crew offline queue | ✅ Done | `/my-tasks` — the field crew's whole product — cold-starts with no signal, and their only two writes queue on the device: a task's status and a daily log. Both are made **safe to replay**, which is the part that is not obvious: a daily log carries a client-generated key so a lost response cannot write it twice (no runtime role holds DELETE on `daily_logs`, so a duplicate would be permanent), and a status change carries the value it was made against, so one queued at 09:00 with no signal cannot silently undo a project manager's decision when it lands at 17:00 |
| 5 — AI takeoff, multi-currency | ⬜ Not scheduled | Per roadmap. Both offline cases — the estimator's and the field crew's — are now built |

Backend test suite (`main`): 1,317 passing tests, plus a 43-test Playwright e2e suite, including dedicated tenant-isolation/RLS regression suites (both a `company_id`-index and an RLS-*policy* coverage gate, each catalog-driven so a future tenant table can't slip past either) and a tier-gating completeness gate. Three CI workflows (six jobs) gate every merge: backend ([backend-ci.yml](.github/workflows/backend-ci.yml) — a `deploy-config` job validating every compose file and the backup scripts, a `docker-build` job that builds the backend image *and runs it* to prove it's serviceable, and a `test` job running pytest against real Postgres 16 + Redis 7, ruff, mypy, and an OpenAPI schema-diff), frontend ([frontend-ci.yml](.github/workflows/frontend-ci.yml) — eslint + typechecked build, plus a `docker-build` job that boots the standalone production image against a stand-in backend), and end-to-end ([e2e-ci.yml](.github/workflows/e2e-ci.yml) — the full stack with a Playwright suite driving real browser flows).

**Remaining gaps** (deliberate, tracked):
- **External-service credentials:** the real clients now exist — `RealStripeClient` (SDK-backed), `RealQuickBooksClient` and `RealFreshBooksClient` — each selected only when its own credentials are configured, with the fakes as the default so tests, CI and a local `docker compose up` make no network calls. What remains is **not code**: a Stripe account, and approved OAuth applications on both accounting providers. Until those exist the integrations exit criterion ("connect and see a successful, monitored sync") cannot be demonstrated end to end. **Email is not in this list any more** — `SmtpEmailClient` is a real implementation (stdlib `smtplib`, run on a worker thread) selected by setting `SMTP_HOST`, and `backend/scripts/send_test_email.py` proves a server's configuration without inviting anybody. What it still lacks is a provider account and DNS: SPF/DKIM/DMARC records are what stop the mail landing in spam, and no application code can supply them. A tenant that HAS published those records can point Builders Stream at their own mail server (Catalog → Email server): credentials encrypted at rest with the integrations key, never returned by any route, and the host checked against private/reserved networks on save and again before every send — a tenant-supplied hostname is an outbound connection made on their behalf, which is SSRF unless somebody says where we may go.
