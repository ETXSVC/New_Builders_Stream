...
| Background Tasks | **Dramatiq + Redis** (as built; this row originally read "Celery or Dramatiq") | PDF generation, QuickBooks/FreshBooks sync, notification delivery. Dramatiq chosen for a materially smaller operational surface; see `backend/app/tasks/broker.py`. |
| Auth | **PyJWT + Argon2id, in-process** (as built; this row originally proposed OIDC via self-hosted Keycloak or Better Auth) | Running an identity server for a single first-party client was not worth its operating cost at this stage. The JWT contract is unchanged, so an OIDC provider remains a swap-in later. See `backend/app/core/security.py` and CLAUDE.md's "deliberate substitutions". |
...
| Reverse Proxy / TLS | **Caddy + Let's Encrypt** (as built; this row originally read "Traefik or Nginx") | Automatic certificate issue and renewal with no sidecar, and spoof-safe `X-Forwarded-For` defaults that the client-IP capture depends on. See `deploy/Caddyfile` and `docs/11-production-deployment.md`. |
...
> **As-built vs. as-designed.** Rows marked **as built** record a choice
> that differs from this document's original proposal. They are kept in
> place, with the original wording named, rather than silently rewritten —
> the reasoning behind a substitution is worth more than a tidy table, and
> a reader comparing this doc to the code should be able to see which of
> the two moved. `backend/openapi.json` is authoritative for the API
> surface; this document is authoritative for intent.
...
| `PROJECT_COMPLETED` | Project Management | Billing — drafts a final invoice for the uninvoiced remainder (**implemented**: `app/services/project_completed_handler.py`). Integrations sync on this event is **not yet implemented**, see [`docs/superpowers/specs/2026-07-15-integrations-quickbooks-freshbooks-design.md`](superpowers/specs/2026-07-15-integrations-quickbooks-freshbooks-design.md) |
