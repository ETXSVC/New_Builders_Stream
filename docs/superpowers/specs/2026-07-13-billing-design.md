...
**Implementation Status (2026-07-14): Complete.** All 16 tasks in [`docs/superpowers/plans/2026-07-13-billing.md`](../plans/2026-07-13-billing.md) are implemented and merged into `feature/billing`. Evidence: full backend suite (`pytest -v` from `backend/`) passed twice solo, 586/586, no failures (1048.75s and 1014.23s runs); the dedicated RLS regression suite (`test_tenant_isolation.py` + `test_rls_policy_regression.py` + `test_tenant_isolation_phase1.py` + `test_tenant_isolation_phase2.py` + `test_tenant_isolation_phase3.py` + `test_billing_tenant_isolation.py`) passed 58/58, confirming no regression in any Phase 0/1/2/Compliance Tracking table from this work; `scripts/e2e_smoke_test.py` was extended with Billing's own exit criterion (trial visibility on registration, portal-session URL generation, non-Admin 403 on `/subscriptions/me`, and a lapsed-subscription 403 on a real write route) and verified passing against the live Docker Compose stack. One out-of-scope infrastructure issue was found during that live-stack verification and deliberately NOT fixed here (flagged separately): the frontend container's host port mapping and the smoke test's own `FRONTEND_URL` have drifted out of sync since an unrelated earlier merge, and the frontend's root page no longer serves the health-check string the script looks for — both pre-existing and unrelated to Billing's own correctness.
...
- **Real Stripe credentials/SDK wiring** is NOT built here. This spec is built entirely behind a `StripeClient` interface with an in-memory fake as the only implementation exercised in tests and wired by default. Swapping in the real `stripe` SDK against live test-mode credentials is a follow-up task, not a code change to this spec's design.
  ...
  **Update (2026-08-01): done.** `RealStripeClient` now exists in `app/services/stripe_client.py`, selected by `STRIPE_API_KEY` alone (`app/services/billing.py`); `FakeStripeClient` remains the default everywhere else. The prediction above held — not one caller changed. Three things the SDK had moved on since this spec was written, each documented at its site: `client.v1.*` replaced the un-namespaced accessors; `current_period_end` left the Subscription object for the subscription *item* in API version 2025-03-31.basil (which had also left `POST /webhooks/stripe` reading a key that no longer arrives — fixed); and the SDK's async methods require `httpx`, previously a dev-only dependency and therefore absent from the production image. Operator steps are `docs/11-production-deployment.md` §6a.
...
- Exact per-tier `included_seats` defaults and Stripe Product/Price configuration are a pending business decision (per `docs/08-pricing-subscription-model.md` Section 2's own note) — placeholder defaults will be used, easily changed via the `included_seats` column without a migration. **Still open as of 2026-08-01**: `TIER_INCLUDED_SEATS` is unchanged, and the Price ids are now configuration (`STRIPE_PRICE_ID_*`) rather than a code decision, so the business answer lands in `.env` rather than in a commit. The overage Price must be **licensed per-unit, not metered** — see `report_seat_usage`, which resolved this spec's other open question by choosing *set* rather than *increment* semantics to match the daily-snapshot job.
- Tier-based module gating retrofit and Invoicing/Expenses/Profitability were each explicitly out of scope here (see Scope section). Module gating and invoicing/expenses have since shipped; real Stripe SDK wiring is done (see the Scope update above). Profitability has a backend (`GET /reports/profitability`) and **no frontend**, which is the live remainder.
