...
**Implementation Status (2026-07-15): Complete.** All 17 tasks in [`docs/superpowers/plans/2026-07-15-integrations-core.md`](../plans/2026-07-15-integrations-core.md) are implemented on `feature/integrations-core`. Evidence: full backend suite (`pytest` from `backend/`) passed twice solo, 709/709, no failures (1071.70s and 1069.68s runs); the dedicated RLS regression suite (the seven prior tenant-isolation/RLS files plus this feature's own `test_integrations_tenant_isolation.py`) passed 73/73, confirming no regression in any earlier phase's tables; `scripts/e2e_smoke_test.py` was extended with this feature's own exit criterion (connect returning a fake authorization URL, callback creating a real connection from a real signed `state` token with no bearer token, sync-status reflecting the connection, and an Invoice creation enqueuing a real Redis sync message) and verified passing against a live Docker Compose stack rebuilt from this branch — including, after a compose fix found by that same live run, the real worker consuming the sync message end-to-end to a `status='success'` row. That live run caught a real deployment bug beyond this feature's own scope: the compose worker command had never been updated for `app.tasks.seat_usage` (Task 3.23), `app.tasks.flag_overdue_financial_records` (Task 3.45), or this feature's `app.tasks.accounting_sync` — all three actors' messages were being silently dead-lettered in the running stack; fixed on this branch with a new unit test (`test_worker_module_registration.py`) enforcing the invariant. A post-spec external design review (cloud ultrareview) surfaced one real functional gap that was fixed on this branch (`handle_estimate_approved` never published `INVOICE_CREATED`, so auto-drafted deposit invoices bypassed sync — the flagship US-6.2 flow) and two forward-compatibility contract gaps (push idempotency, token refresh) now explicitly documented in this spec's Open Questions as requirements the real-provider follow-up spec inherits. The same pre-existing, out-of-scope frontend port-mapping issue flagged since Task 3.29 was encountered again and deliberately NOT fixed here (on this host, ports 3001-3100 additionally sit in a Windows excluded port range — environmental, unrelated to this feature's correctness).
...
- **Real `RealQuickBooksClient`/`RealFreshBooksClient` implementations** — **built 2026-08-01** (`app/services/quickbooks_client.py`, `app/services/freshbooks_client.py`, migration 0030). Both inherited requirements below were implemented as stated. Three things this section did **not** anticipate, each of which changed the Protocol or the schema rather than living inside an implementation:
  - **An account id per connection.** QuickBooks returns a `realmId` as a query parameter on the OAuth *callback* — not in the token response — and it is a path segment of every request; FreshBooks needs an `accountId` fetched from `/auth/api/v1/users/me`. The callback route was discarding the former. Migration 0030 adds `integration_connections.provider_account_id`; a connection made before it must be reconnected.
  - **Entity resolution.** A QuickBooks invoice cannot exist without a `CustomerRef` and a bill without a `VendorRef`, and `invoices` has no client column at all — the payee is resolved through `project_clients`. Migration 0030 adds `integration_entity_mappings` to cache what each local name resolved to, because `resolve_entity` is find-or-**create** and forgetting means creating a second customer in somebody's real books.
  - **The two providers disagree about what a record needs.** A QuickBooks invoice line requires an `ItemRef`; a FreshBooks line is free text. So each client *declares* its requirements (`required_refs`) and the sync actor satisfies them generically, rather than the actor hardcoding one provider's shape.
  - One claim below is **narrower than it reads**: QuickBooks does implement client-supplied idempotency (a `requestid` query parameter, not a header). FreshBooks has **no equivalent**, so its client searches for a record already tagged with the sync key before creating one — a genuinely weaker guarantee, documented at `freshbooks_client.py`'s own docstring.

  Original text, kept because it is what the implementation was built against:
...
- **PROJECT_COMPLETED and INVOICE_PAID as sync triggers** — deferred until those events themselves are implemented (neither exists in the codebase today); syncing on Invoice/Bill *payment* or Project *completion*, not just creation, is a real gap this spec knowingly leaves for later.
  ...
  **Update (2026-08-01): both closed, and the invoice half is named differently on purpose.** `PROJECT_COMPLETED` gained a handler that re-publishes `INVOICE_CREATED`, so completion-generated invoices sync. The payment half ships as **`INVOICE_PAYMENT_RECORDED`, published per payment**, rather than the `INVOICE_PAID` named here: an event that only fires when an invoice settles would never sync a partial payment, so an invoice paid in three instalments would show as entirely unpaid in the tenant's books until the last one landed, then jump to paid in full with the first two payments absent. The entity carried is the `invoice_payments` row.

  Two consequences worth knowing. `push_payment` joined the Protocol, and a payment **cannot sync before its invoice has** — QuickBooks needs a `LinkedTxn` and FreshBooks an `invoiceid`, or the money lands as an unapplied credit while the invoice still reads as outstanding. The actor reads the invoice's own `external_record_id` and fails the payment visibly if it is absent, naming which to fix first. Migration 0032 widened `integration_sync_records`' `entity_type` CHECK, which is what caught this needing a migration at all.
