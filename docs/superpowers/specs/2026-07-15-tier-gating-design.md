...
**Implementation Status (2026-07-16): Complete.** All 10 tasks in [`docs/superpowers/plans/2026-07-15-tier-gating.md`](../plans/2026-07-15-tier-gating.md) are implemented on `feature/tier-gating`. Evidence: full backend suite (`pytest` from `backend/`) passed twice solo, 743/743, no failures (1148.51s and 1154.61s runs); the dedicated RLS regression suite (the seven prior tenant-isolation/RLS files plus `test_integrations_tenant_isolation.py`) passed 73/73, confirming tier gating regressed no earlier phase's row-level security — including the child-branch cases that depend on the `subscriptions` upward-visibility policy for root-tier resolution; `scripts/e2e_smoke_test.py` was extended with this feature's own live exit criterion (a trialing/pro company receives a 403 naming the enterprise plan from `GET /integrations/quickbooks/connect`, then passes after a raw-SQL tier bump — plus enterprise bumps for the pre-existing child-branch-creation and Invoicing blocks that this feature's own gates now cover) and verified passing against a live Docker Compose stack rebuilt from this branch, 74/74 backend checks. One authorized scope addition beyond the plan's original route list, found by the Task 5.4 review: the compliance-expiry scheduler (owner-role, no request context) kept minting notifications for below-tier companies that could never dismiss them (dismiss is Pro+-gated); fixed in Task 5.8 with a memoized `tier_allows` skip that preserves fail-open for companies with no subscription row. Known-and-deliberately-not-fixed items, all Minor: the 403 message ("This feature requires the pro plan") on estimate approve/reject is also shown to the signing *client*, for whom "upgrade your plan" is aimed at the wrong party — wording is a product decision deferred to the frontend UX pass the spec already defers; stale "for Pro" comments in `backend/tests/test_seat_usage_task.py` (~lines 80/87/135) predate this feature; `test_tier_gating.py` imports six helpers from `tests/test_estimate_approved_handler.py` — acceptable at two files, promote to `conftest.py` if a third consumer appears; the tier-gated event handlers skip silently by design (no log line) — add observability if support burden ever demands it. The same pre-existing, out-of-scope frontend port-mapping issue flagged since Task 3.29 (ports 3001+ in a Windows excluded port range on this host) was encountered again in the live E2E run and deliberately NOT fixed here.
