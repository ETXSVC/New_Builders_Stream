...
**Implementation Status (2026-07-16): Complete.** All 9 tasks in [`docs/superpowers/plans/2026-07-16-auth-token-lifecycle.md`](../plans/2026-07-16-auth-token-lifecycle.md) are implemented on `feature/auth-hardening`. Evidence: full backend suite passed twice solo, 757/757, no failures (1346.52s and 1309.23s); the 8-file RLS regression suite passed 73/73; `scripts/e2e_smoke_test.py` extended with the full lifecycle over real HTTP (rotation with `Cache-Control: no-store` asserted, spent-token reuse 401, family-containment 401 for the fresh successor, logout 204 then 401) and verified against a live compose stack rebuilt from this branch, 80/80 backend checks (the known pre-existing frontend port-3001 issue remains, untouched). The per-task review pipeline caught and fixed five substantive issues beyond the plan: a missing CHECK constraint guarding the replaced-but-not-revoked double-spend state (proven live); a concurrent double-rotate race under READ COMMITTED that silently bypassed reuse detection (fixed with `SELECT ... FOR UPDATE` on the rotation lookup, plus a deterministic gather-race test); a false transaction-semantics claim justifying the refresh route's original shape (restructured to commit-then-raise); a third verbatim copy of the default-membership query (extracted to `_default_membership`, the Decision 9 invariant now lives in one place); and Task 6.4 fallout where hard-deleting a user tripped the new FK (ondelete=CASCADE, matching company_users' precedent). Full regression also caught the three new routes missing from the read-only completeness test's documented exclusion list — excluded with rationale per that test's own maintenance contract. One test deviation was adjudicated against the plan itself: presenting a family-killed token is a second audited reuse event, so the audit-count assertion is a stricter two-point check. Additions beyond the plan, all review-driven: `Cache-Control: no-store` on both token responses (RFC 6749 §5.1). Known-and-deliberately-not-done, beyond the Deferred section below: no-store on token-endpoint *error* responses; rate limiting on `/auth/refresh` (one leaked dead token permits unbounded audited reuse events — each arguably signal, but unthrottled); the reuse-branch 401 is marginally slower than the unknown-token 401 (a timing oracle only for someone who already holds a real token).
