...
- Writes an audit log entry (`bom_line.auto_generated` or similar) per created/updated line, `actor_id=None` for the same documented reason the existing handler uses (`ESTIMATE_APPROVED`'s publish payload carries no actor).

For each `EstimateLineItem` on the approved estimate that has a `cost_catalog_item_id`: look up an existing `BomLine` for `(project_id, cost_catalog_item_id)`. If one exists (a later estimate needs the same material), add this estimate's `quantity` to it. Otherwise create a new line with `source="estimate"`, `quantity` from the line item, `description`/`unit` snapshotted from the catalog item. Line items with no `cost_catalog_item_id` (there are none in the current schema, but the field is nullable at the `EstimateLineItem` level in principle) are skipped — the BOM only tracks catalog-backed materials from estimates; ad-hoc estimate lines don't have a reliable identity to merge against.
