...
- `ProjectClientDashboardResponse`: `id`, `name`, `status`, `site_address`, `projected_start_date` — no `lead_id` (internal CRM linkage, not the client's business), matching design decision #8's "no internal operational detail" framing.
...
- `DocumentUploadRequest`/`DocumentResponse` — the upload itself is `multipart/form-data`, not a JSON body; design the request handling accordingly (FastAPI `UploadFile`), the "schema" here is really the response shape plus the `file_name` form field.
...
- `POST /projects`: `require_role("admin", "project_manager")`. Manual initialization path (independent of `LEAD_WON` — Task 1.16 wires the automatic path).
...
- `GET /projects/{id}`: role-based response shape per design decision #8. 404 for cross-tenant.
...
- `PATCH /tasks/{id}`: `admin`/`project_manager` can update anything; `field_crew` can update `status` only on tasks assigned to them (`assignee_id == current.user.id`) — this is a genuinely new RBAC shape (role AND ownership, not just role), test it explicitly both ways (field_crew updating their own task succeeds, field_crew updating someone else's task is blocked).
- Tests: phase creation, task creation, cross-project phase_id rejected, task patch as PM (any task), task patch as field_crew (own task succeeds, others' task 403), cross-tenant 404s.
- Verify: full suite green.
...
- Tests: upload, upload-new-version-of-same-filename (both retrievable), path-traversal-rejected, cross-tenant 404, list shows latest-version-only by default.
- Verify: full suite green.
