...
**Known gap to record in the closeout note (Task 13 review):** `next.config.ts` has no `reactStrictMode` override, so it defaults to `true`. In `next dev` (not production builds), Strict Mode double-invokes effects on mount specifically to surface missing-cleanup bugs — `AuthContext`'s mount-time hydration effect (Task 11's fix) does have a `cancelled` guard, so only the second invocation's result is ever applied to state, but BOTH invocations still fire a real `POST /api/auth/refresh` network call using the same refresh_token cookie. That's the same single-use-rotation-plus-reuse-detection mechanism already documented as the multi-tab race (`backend/app/services/refresh_tokens.py`) — except reachable here within a single tab, dev-server-only. Shares its root cause and likely its fix (a request-generation counter in `AuthContext`) with the two already-documented `AuthContext` gaps (the multi-tab race, and `clearSession`'s in-flight-refresh race) — flagging alongside them as the same class of fast-follow, not fixing individually here.
