...
- Multi-factor authentication (TOTP) is a requirement for the Admin role at minimum, strongly recommended for all roles — should be scoped into an early phase given the platform handles client financial/contract data. Implemented per [`docs/superpowers/specs/2026-07-16-mfa-totp-design.md`](superpowers/specs/2026-07-16-mfa-totp-design.md): self-service enroll/activate/disable available to every role, replay-guarded RFC 6238 verification, and disabling MFA revokes every refresh token the user holds (the same posture as a password change). The Admin "requirement" is enforced in v1 as a server-side `mfa_enrollment_required` signal on login/refresh, not a hard login block — a hard block would deadlock a fresh registration's first-ever admin before they can enroll; the frontend is expected to force the enrollment screen on that signal. Recovery codes are deferred; the v1 answer to a lost authenticator is an owner-role SQL reset (documented in the spec).
